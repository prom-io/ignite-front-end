...
const igniteDescriptionTranslations = {
    en: classes => (
        <span className={classes.content}>
            <a>Ignite </a> is a decentralized social network, based on blockchain
            technology. You have to use a special ID named{" "}
            <a>blockchain wallet address</a> to sign up.
        </span>
    ),
    kr: classes => (
        <span className={classes.content}>
            <a>Ignite </a>는 블록체인 기술을 기반으로 한 분산형 소셜 네트워크다.
            가입하려면
            <a>블록체인 지갑주소 라는</a> 특수 아이디를 사용해야 합니다
        </span>
    )
};
...
export const SignUp = observer(() => {
    ...
    return (
        <DialogContent>
            ...
            <div className={classes.contentBlock}>
                <div>{useExistingWalletTranslations[locale]()}</div>
                <Button
                    variant="outlined"
                    color="primary"
                    classes={{
                    ...
                    onClick={() =>
                        setGenericAuthorizationDialogType("createAccount")
                    }
                >
