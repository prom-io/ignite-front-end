...
const errorTranslations = {
    ...
    kr: classes => (
        <div className={classes.contentDescription}>
            뭔가 잘못 된 것 같아요… 연락주세요{" "}
            <a onClick={() => window.open("http://ignite.so/")}>Ignite.so</a> 아니면{" "}
            <a onClick={() => window.open("http://prometeus.so/")}>Prometeus.io</a>.
    ...
    return (
        <DialogContent>
            ...
            <div className={classes.notes}>
                <a>{l("sign-up.note")}</a> {l("sign-up.keep-your-login")}
            </div>
            <div className={classes.descriptionBold}>{l("sign-up.sorry")}</div>
        </DialogContent>
    );
});
