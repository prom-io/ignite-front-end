...
const tryOurNetworkTranslations = {
  en: ({classes}) => (
      <Typography className={classes.styledText}>
          Try <a className={classes.prometeusLink} href="https://prometeus.io" target="_blank noopener noreferrrer">Stoa</a>, which allows to buy and sell any imaginable digital data: contract templates, music, 3D models, source codes, stats or your master's thesis – anything you can think of, saved in a file.
      </Typography>
  ),
  ko: ({classes}) => (
      <Typography variant="body2">
          를 사용해 보십시오. 계약 템플릿, 음악, 3D 모델, 소스 코드, 통계 또는 석사 논문 등 상상할 수 있는 모든 디지털 데이터를 구입하고 판매할 수 있는 <a className={classes.prometeusLink} href="https://prometeus.io" target="_blank noopener noreferrrer">Stoa</a>를 파일로 저장합니다.
      </Typography>
  )
};
    ...
    return (
        <Grid container spacing={2} className={classes.bannerContainer}>
            ...
            <div className={classes.descriprionLinks}>
              <p><StaticPageLinks linkTekst={links.termsOfService} routerStore={routerStore} targetView={Routes.terms}/> &bull;<StaticPageLinks /> <StaticPageLinks linkTekst={links.privacyPolicy} routerStore={routerStore} targetView={Routes.terms}/></p>
              <p>© 2020 {Prometeus} Team</p>
            </div>
        </Grid>
    )
};
